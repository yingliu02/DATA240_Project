...
class EnhancedChurnPredictor(pl.LightningModule):
    def __init__(self, input_size, hidden_size, output_size, batch_size, nheads):
        super(EnhancedChurnPredictor, self).__init__()
        self.layer1 = nn.Linear(input_size, hidden_size)
        self.batch_norm1 = nn.BatchNorm1d(hidden_size)
        self.dropout1 = nn.Dropout(p=0.3)
        
        # Adding a Transformer Encoder layer
        encoder_layers = TransformerEncoderLayer(hidden_size, nheads)
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.batch_norm2(self.layer2(x))) + residual  # Adding residual
        x = self.dropout2(x)
...
if __name__ == '__main__':
    model = EnhancedChurnPredictor(input_size=14, hidden_size=20, output_size=1, batch_size=64, nheads=2)
